/* ============================================================
   ...
   APIs: apiTramConsultarMoto, apiTramEscribirFecha, etc.
   ============================================================ */
...
/* ============================================================
   ...
   ============================================================
   Esta es la vista más compleja del app. Maneja:
   - Header sticky con info de moto
   - Días expandibles
   - Steps con check, registro de fecha, alistamientos
   ============================================================ */
function pTrackView() {
  if (pLoading) {
  ...
  DAYS.forEach(function(day) {
    ...
    if (isOpen) {
      ...
      steps.forEach(function(step) {
        ...
        // Find matching TRAM_STEP for date registration
...
/* ============================================================
   Helper: encuentra el TRAM_STEP correspondiente a un step
   de DAYS, por título exacto o por fuzzy matching
   ============================================================ */
function pFindTramStep(step) {
  ...
  // Fuzzy match para steps de trámites cuyo título difiere
  if (step.c === 'tra') {
    for (var ti = 0; ti < TRAM_STEPS.length; ti++) {
...
/* ============================================================
   ...
   ============================================================
   También envía a la lista SharePoint Registro_Actividades.
   ============================================================ */
function pCheckStep(dayNum, stepIdx) {
  ...
  // Enviar a Lista SharePoint
  var step = day.steps[stepIdx];
  var actNum = TRAM_ACT_NUM_MAP[step.t] || null;
  if (actNum && getUrl('tramEscrAct')) {
    apiAvanceEscribir({
      ...
      dia: 'Día ' + dayNum,
      estado: 'Ejecutada',
      responsable: pUserArea ? (AREAS[pUserArea] ? AREAS[pUserArea].l : '') : '',
      fecha_registro: fIso(),
      comentario: ''
    }).catch(function() {
      // Falla silenciosa
